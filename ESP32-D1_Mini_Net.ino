...
2.開發版：WEMOS D1 MINI version_3.3.5
...
// 網路配置
String ap_ssid = "ESP32_D1_MINI_Config";
...
void setupWebServer()
{
  ...
  server.on("/api/ap/config", HTTP_POST, handleSetApConfig);
  ...
  server.on("/", HTTP_GET, []()
    ...
    if (SPIFFS.exists("/index.html")) {
      ...
      String message = "ESP32_D1_MINI 網路配置系統\n\n";
      message += "目前模式: ";
      switch(currentMode) {
...
void setupOTA()
{
  ...
  ArduinoOTA.setHostname("ESP32_D1_MINI");
...
void handleSetApConfig()
{
  if (server.hasArg("plain"))
  {
    ...
    if (!error)
    {
      ...
      saveConfigToSPIFFS(); // 寫入 config.json

      server.send(200, "application/json", "{\"success\":true,\"message\":\"AP設定已更新\"}");
    }
    else
    {
      server.send(400, "application/json", "{\"success\":false,\"message\":\"JSON解析失敗\"}");
    }
  }
}